...
public class MainActivity extends Activity {
    ...
    private TextView tv(String text, float size, int color, boolean bold) {
        ...
        // Do not force every string into RTL. Mixed Arabic/English strings
        // were causing glyphs and words to overlap in the old UI.
    ...
    private TextView rtlTv(String text, float size, int color, boolean bold) {
        TextView t = tv(text, size, color, bold);
        t.setTextDirection(View.TEXT_DIRECTION_RTL);
        t.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        return t;
    }

    private TextView ltrTv(String text, float size, int color, boolean bold) {
        TextView t = tv(text, size, color, bold);
    ...
    private void showHome() {
        ...
        counter.setPadding(18,8,18,4);
        counter.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        counter.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
        countText=rtlTv(String.valueOf(devices.size()),18,blue,true);
        TextView a=rtlTv("الأجهزة المكتشفة :",17,dark,false);
        counter.addView(countText,new LinearLayout.LayoutParams(48,48));
        counter.addView(a,new LinearLayout.LayoutParams(-2,48));
    ...
    private void addCard(Device d) {
        ...
        n.setTextDirection(containsArabic(name) ? View.TEXT_DIRECTION_RTL : View.TEXT_DIRECTION_LTR);
        n.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        TextView ip=rtlTv("عنوان الأيبي : "+(d.ip.isEmpty()?"غير متاح":d.ip),15,dark,false);
        TextView mac=rtlTv("عنوان الماك : "+(d.mac.isEmpty()?"غير متاح":d.mac),15,dark,false);
        info.addView(n,new LinearLayout.LayoutParams(-1,42));
        info.addView(ip,new LinearLayout.LayoutParams(-1,36));
        info.addView(mac,new LinearLayout.LayoutParams(-1,36));
    ...
    private void showMenu() {
        ...
        panel.setPadding(16,16,16,20);
        panel.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);

        LinearLayout closeRow=new LinearLayout(this);
        closeRow.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
        TextView close=ltrTv("×",38,Color.rgb(180,180,180),false);
        close.setGravity(Gravity.CENTER);
        close.setOnClickListener(v->pw.dismiss());
        closeRow.addView(close,new LinearLayout.LayoutParams(56,56));
        panel.addView(closeRow,new LinearLayout.LayoutParams(-1,60));

        TextView logo=ltrTv("ALSAMAN",30,dark,true);
        logo.setGravity(Gravity.CENTER);
        panel.addView(logo,new LinearLayout.LayoutParams(-1,72));

        Space gap=new Space(this);
        panel.addView(gap,new LinearLayout.LayoutParams(1,10));
        ...
        pw.setContentView(panel);
        pw.setWidth((int)(getResources().getDisplayMetrics().widthPixels*0.72f));
        pw.setHeight(-1);
        pw.setBackgroundDrawable(bg(Color.WHITE,0));
        pw.setOutsideTouchable(true);
        pw.setFocusable(true);
        pw.setClippingEnabled(true);
        pw.showAtLocation(getWindow().getDecorView(),Gravity.RIGHT,0,0);
    }
    ...
    private View menuItem(String s,String ico,View.OnClickListener l) {
        ...
        TextView icon=ltrTv(ico,27,gray,false);
        icon.setGravity(Gravity.CENTER);
        TextView text=rtlTv(s,19,dark,false);
        text.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        text.setMaxLines(1);
        text.setEllipsize(android.text.TextUtils.TruncateAt.END);

        row.addView(icon,new LinearLayout.LayoutParams(58,70));
        row.addView(text,new LinearLayout.LayoutParams(0,70,1));
    ...
    private void showSetup() {
        ...
        TextView h=rtlTv("خطوات مهمة لإعداد جهازك",28,dark,true); h.setGravity(Gravity.CENTER); body.addView(h,new LinearLayout.LayoutParams(-1,65));
        TextView sub=rtlTv("أجهزة اوبن وورت",19,dark,true); body.addView(sub);
        String[] steps={
    ...
    private void showBreed() {
        ...
        TextView h=rtlTv("الدخول إلي البريد ريكفري",27,dark,true); h.setGravity(Gravity.CENTER); root.addView(h,new LinearLayout.LayoutParams(-1,65));
        TextView d=rtlTv("يتم استخدام هذه الأداة للدخول إلى وضع الريكفري للأجهزة التي تحتوي على بوت لودر البريد.",17,dark,false); d.setGravity(Gravity.CENTER); d.setPadding(35,0,35,0); root.addView(d);
    ...
    private void showSettings() {
        ...
        b.addView(rtlTv("إعدادات الاكتشاف",22,blue,true));
        b.addView(toggle("اكتشاف أجهزة Ubnt",true));
        b.addView(toggle("اكتشاف أجهزة ROS",true));
        b.addView(line()); b.addView(rtlTv("التطبيق والمظهر العام",22,blue,true));
        b.addView(rtlTv("حدد سمة التطبيق",18,dark,false));
    ...
    private void showAbout() {
        ...
        TextView join=rtlTv("انضم إلينا",27,dark,false);join.setGravity(Gravity.CENTER);root.addView(join);
    ...
    private void showSearch() {
        ...
        TextView result=rtlTv("سيتم البحث داخل الأجهزة المكتشفة.",17,gray,false);result.setGravity(Gravity.CENTER);root.addView(result);
